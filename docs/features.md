...
**BigQuery — background analytics now supported.** BigQuery connections work for interactive "run a query" (chat + SQL panel) and unattended background analytics (dashboards/metrics/reports) within a configurable daily byte budget and optional offline/cached mode. Remaining intentionally-blocked features (pending demand) include: data profiling, anomaly detection, data-quality/health checks, the query accelerator (OLTP-only; BigQuery has its own caching), mine query history, MCP tool execution, scheduled queries (cron scheduler itself), the data-drift monitor, notebook re-run, schema-browser query execution, the eval harness, and a bookmark's re-run action. Each surfaces a clear "not yet supported for BigQuery connections" error instead of running. This is a cost-safety scope decision, not a bug — intentionally narrow until there's demand for a budgeted/metered version of any of these.
