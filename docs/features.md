...
- **Query accelerator** (opt-in) — for heavy read queries (>100K row estimate), route through a cached Parquet snapshot of the referenced tables instead of the live driver for lower latency and reduced DB load. Set `accelerateEnabled` + optional `accelerateTtlMs` (snapshot cache TTL) per connection. Results show a badge indicating staleness window; single ANSI-standard SELECTs only — complex queries silently fall back to live execution.
