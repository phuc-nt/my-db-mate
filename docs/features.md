...
- **Five engines + cloud** — PostgreSQL, MySQL/MariaDB, SQLite, SQL Server / Azure SQL, BigQuery, and Cloudflare D1 (remote), via a pluggable connection-provider abstraction. A **provider preset** picker pre-fills the port / SSL mode / quirks for common managed databases (see the compatibility table below).
...
- **BigQuery cost-safety (2 layers)** — BigQuery bills by bytes scanned, so every query on a BigQuery connection is estimated before it runs, via a `dryRun` job (free, no bytes billed) that returns `totalBytesProcessed`. The UI shows the estimated GB + a rough USD cost and requires an explicit confirm click before the real query runs (layer 1, UX-level). Independently, every real query job carries a per-connection `maximumBytesBilled` cap (default 1 GiB, configurable) — **BigQuery itself rejects the job before running it** if it would exceed the cap, and a rejected job is never billed. This second layer is the actual backstop: it holds even if the dry-run estimate is wrong or a caller skips the confirm UI, because it's enforced server-side by BigQuery, not by this app. Known limitation: the dry-run estimate can be unreliable (reported as 0 bytes) for queries hitting external/federated tables — the UI flags this explicitly when it happens, but `maximumBytesBilled` still applies regardless. BigQuery connections are execution-only in v1 — see [Deferred](#deferred) for the full list of app features that intentionally don't run against BigQuery.
...
| BigQuery | REST (service account) | n/a | ✓ | Cost-based, not wire-compatible — own connector. Auth via service-account JSON key (`roles/bigquery.dataViewer` + `roles/bigquery.jobUser`, no write role). Every query dry-run estimated + confirmed, plus a hard `maximumBytesBilled` cap enforced by BigQuery itself. Execution-only in v1 (see [Deferred](#deferred)). |
...
**BigQuery — execution-only in v1.** BigQuery connections work for the interactive "run a query" flow (chat + the SQL panel's edit/re-run), which is the only path wired through the dry-run-estimate + confirm UX. Every other feature that would run BigQuery SQL unattended or on a schedule — where nobody is present to see or approve a cost estimate — is deliberately blocked rather than silently metered: data profiling, anomaly detection, the query accelerator (snapshot/incremental-cache maintenance), mine query history, MCP tool execution, scheduled reports, the data-drift monitor, dashboard pin/refresh, report generation, notebook re-run, schema-browser query execution, the eval harness, and a bookmark's re-run action. Each surfaces a clear "not yet supported for BigQuery connections" error instead of running. This is a cost-safety scope decision, not a bug — a future plan can revisit case-by-case if there's demand for a budgeted/metered version of any of these.
