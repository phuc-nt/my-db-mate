...
3. Với DB cloud (Neon/Supabase/RDS/PlanetScale): tick **SSL** (dán URL có `?sslmode=require` sẽ tự tick).
...
- **TLS không verify cert** — tick SSL sẽ **mã hoá** đường truyền nhưng **không** verify certificate của server (`rejectUnauthorized: false`), nên cloud có CA riêng kết nối được ngay. `sslmode=verify-full`/`verify-ca` hiện được xử lý như `require` (mã hoá, không verify). Ổn trên mạng nội bộ/LAN; nếu tunnel qua internet công cộng, lưu ý kênh **không** chống được MITM.
