...
6. Bấm **Test connection** rồi **Add & sync**.
...
- **BigQuery — chỉ chạy query trực tiếp (chat/execute) trong v1.** Vì BigQuery tính tiền theo byte scan, các tính năng nội bộ chạy nền/định kỳ (không có người xác nhận chi phí) **cố ý bị chặn** cho connection BigQuery: data profiling, anomaly detection, accelerator (snapshot/incremental-cache), mine query history, MCP tool execution, chat agent tự động, scheduled reports, data monitor, dashboard pin, report generation, notebooks, chạy query từ schema browser, eval harness, và nút re-run trên bookmark. Mỗi tính năng trên báo lỗi rõ ràng ("chưa hỗ trợ BigQuery") thay vì chạy ngầm và phát sinh phí ngoài kiểm soát. Đây là quyết định phạm vi có chủ đích cho v1, không phải thiếu sót.
