...
1. Chọn engine: **PostgreSQL / MySQL(MariaDB) / SQLite / SQL Server / BigQuery** (hoặc **Cloudflare D1** remote). Với SQL Server, dùng login chỉ có `db_datareader` để read-only thật, và cấp thêm `GRANT SHOWPLAN` (quyền metadata-only) để hệ thống ước lượng được chi phí query — thiếu quyền này thì mọi query đều bị hỏi xác nhận trước khi chạy.
...
### Kết nối BigQuery (chi phí thật — đọc kỹ)

BigQuery tính tiền theo **số byte scan**, khác hẳn các engine OLTP ở trên (miễn phí theo compute). Form kết nối vì vậy cũng khác:

1. Chọn engine **BigQuery**, điền **GCP project ID**.
2. Tạo một **service account** trên GCP Console (IAM & Admin → Service Accounts), cấp đúng 2 role tối thiểu:
   - `roles/bigquery.dataViewer` — đọc dữ liệu (nên cấp ở **cấp dataset**, không phải project, để giới hạn phạm vi; cấp dataset-level nghĩa là app chỉ liệt kê được các dataset đã được cấp quyền — đây là hành vi đúng, không phải bug).
   - `roles/bigquery.jobUser` — quyền chạy query job (không phải quyền ghi dữ liệu).
   - **Không cấp** role Editor/Writer nào. Lưu ý: `jobUser` + `dataViewer` vẫn cho phép BigQuery tự tạo bảng đích tạm/ẩn danh như một cơ chế nội bộ khi chạy query (tự hết hạn, tính vào số byte đã scan) — đây không phải là quyền ghi vào dataset thật của bạn.
3. Tạo **JSON key** cho service account đó (Keys → Add key → JSON), tải file về.
4. Dán **toàn bộ nội dung file JSON** vào ô service-account JSON trên form (khi sửa connection sau này, để trống ô này sẽ giữ nguyên key cũ).
5. Điền **Max bytes billed per query** (mặc định 1 GiB) — đây là **ngưỡng cứng**: BigQuery tự chặn job trước khi chạy thật nếu query cần scan nhiều hơn ngưỡng này, **không tính phí** cho job bị chặn. Đây là lớp bảo vệ chính, độc lập với UI xác nhận (xem [features.md](features.md) mục cost-safety).
6. Điền **Daily byte budget** (mặc định 10 GiB/ngày ≈ $0.06/ngày) — **ngưỡng tổng byte scan mỗi ngày cho dashboards/metrics/reports chạy nền**: refresh nào sẽ vượt quá ngân sách hôm nay sẽ bị bỏ qua (không chạy), chứ không phát sinh phí ngoài kiểm soát. Dùng reserve-then-reconcile nên các refresh song song không thể tổng cộng vượt ngân sách. Đặt thành 0 để chặn hoàn toàn background BigQuery.
7. Bật **Offline mode** (tuỳ chọn) nếu muốn dashboards/metrics/reports dùng cache DuckDB thay vì live query: 1 lần extract từ BigQuery (vẫn tuân theo ngân sách ngày + per-query cap), rồi đọc $0 từ cache tới khi snapshot hết hạn (mặc định 6 giờ). Dữ liệu cache/cũ giữa các lần refresh; mốc thời điểm cache được ghi nhãn "as of".
8. Bấm **Test connection** rồi **Add & sync**.

**Khuyến nghị thêm (ngoài phạm vi app, cấu hình thủ công trên GCP):** đặt thêm một **custom quota theo ngày** cho BigQuery API trên GCP Console (IAM & Admin → Quotas, lọc "BigQuery API", tìm quota "Query usage per day" theo project) — đây là lớp chặn cấp hạ tầng, hoạt động độc lập với app (kể cả khi app có bug). App không tự động hoá bước này vì Service Usage API cho việc set quota chưa đủ ổn định để tích hợp an toàn ở v1.

Mỗi connection mở thành **một workspace** tại `/db/<id>` với thanh section: **💬 Chat · 🗂 Schema · 📚 Context · 📈 Metrics · ⚡ Accelerator · ⏰ Automations** (link cũ Chat/Browse/Context tự chuyển hướng). Nav trên cùng gọn còn: **Connections · Library · ⚙ Settings**.

- **⚡ Accelerator** (tab luôn hiện, mờ + nhãn "off" khi chưa bật): nếu DB có bảng lớn, tick **Enable query accelerator** để tăng tốc query nặng — hệ thống cache bảng thành Parquet file và chạy query trên snapshot (thay vì DB trực tiếp), giảm tải. Điền TTL cache (mặc định 1 giờ) — kết quả sẽ ghi nhãn "⚡ Accelerated · snapshot …" để bạn biết data cũ bao lâu. Chỉ áp dụng cho simple SELECT (không CTE, không function lạ); query phức tạp tự chạy bình thường.
  ...
  - Tab này cũng hiện **danh sách snapshot** (SQL/bảng, thời điểm cache, dung lượng, trạng thái, lỗi gần nhất) kèm nút **Refresh** thủ công từng dòng.
  - **Tải từng phần (incremental refresh)**: cùng tab này, chọn một bảng — hệ thống sẽ gợi ý một cột timestamp (`created_at`, `updated_at`, `modified_at`, v.v.) nếu tìm thấy. Nếu chọn "Confirm", mỗi lần TTL hết chỉ cập nhật dòng mới (thay vì tải lại toàn bộ bảng) — giảm tải đáng kể với bảng lớn. Bạn có thể bật/tắt riêng từng bảng, không liên quan đến cài đặt accelerator chung của connection.
...
- Hỗ trợ PostgreSQL, MySQL/MariaDB, SQLite, SQL Server, Cloudflare D1, và BigQuery (chỉ chat/chạy query trực tiếp — xem giới hạn ở mục 4).
...
- **BigQuery — background analytics giờ hỗ trợ.** Dashboards/metrics/reports giờ chạy cho BigQuery trong ngân sách byte hàng ngày (+ offline mode cache tuỳ chọn). Các tính năng còn lại **cố ý bị chặn** cho đến khi có nhu cầu: data profiling, anomaly detection, data-quality/health checks, accelerator (BigQuery có cơ chế cache riêng), mine query history, MCP tool execution, scheduled-query scheduler (cron), data-drift monitor, notebook re-run, chạy query từ schema browser, eval harness, và nút re-run trên bookmark. Mỗi tính năng trên báo lỗi rõ ràng ("chưa hỗ trợ BigQuery") thay vì chạy ngầm. Đây là quyết định phạm vi có chủ đích, không phải thiếu sót.
