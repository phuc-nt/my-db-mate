  ...
  describe('renderContextForPrompt — metrics block', () => {
    ...
    it('includes a "Governed metrics" block when metrics are present', () => {
      const rendered = renderContextForPrompt({
        ...emptyCtx,
        metrics: [{ id: 'test-metric', name: 'Monthly Revenue', description: 'total sales revenue per month', sql: 'SELECT 1 UNION ALL SELECT 2', dimensions: null, distance: 0 }],
      });
    ...
    it('renders metric without dimensions when dimensions is null', () => {
      const rendered = renderContextForPrompt({
        ...emptyCtx,
        metrics: [{ id: 'test-metric', name: 'Simple Count', description: 'simple count metric', sql: 'SELECT COUNT(*) FROM t', dimensions: null, distance: 0 }],
      });
    ...
    it('renders metric with empty description and with dimensions', () => {
      const rendered = renderContextForPrompt({
        ...emptyCtx,
        metrics: [{ id: 'test-metric', name: 'No Desc Metric', description: null, sql: 'SELECT 1', dimensions: ['region', 'channel'], distance: 0 }],
      });
  ...
  describe('mixed context integration', () => {
    it('renders both metrics and verified examples when both are present', () => {
      const rendered = renderContextForPrompt({
        ...
        metrics: [{ id: 'test-metric', name: 'Churn Rate', description: 'monthly churn percentage', sql: 'SELECT 1', dimensions: null, distance: 0 }],
      });
      expect(rendered).toContain('Governed metrics');
